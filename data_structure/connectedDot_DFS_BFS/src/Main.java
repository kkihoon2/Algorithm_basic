...
public class Main {
    public static void main(String[] args) {
        ...
        int result = k.connectedVertices(new int[][]{
                {0, 1},
                {2, 3},
                {4, 5},
        });
        System.out.println(result); // 3
        int result2 = k.connectedVertices(new int[][]{
                {0, 1},
                {2, 3},
                {3, 4},
                {3, 5},
 ...
 class Solution {
    public int connectedVertices(int[][] edges) {
        // TODO: 방향이 없는 간선들의 목록이 주어질 때, 연결된 정점의 컴포넌트(그룹들)가 몇 개인지 반환하는 함수를 작성하세요.
        int len = 0 ;
        ...
        for(int v = 0 ; v<visited.length;v++){
            if(!visited[v]){
                cnt++;
                visited=bfs_array(arr,v,visited);
            }
        }
        return cnt;
    }
    public boolean[] bfs_array(int [][] arr,int v,boolean[]visited)
    {
